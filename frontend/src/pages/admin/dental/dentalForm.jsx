...
export default function DentalRecordForm({ onBack, onSave }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto w-full max-w-5xl rounded-2xl border border-border bg-surface p-8 shadow-lg">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 border-t border-border pt-6 sm:grid-cols-3">
        <div>
          <FieldLabel htmlFor="last_visit">When was the last time you visited a dentist?</FieldLabel>
          <input id="last_visit" name="last_visit" type="text" placeholder="e.g. 6 months ago" className="w-full rounded-xl border border-border bg-background px-3.5 py-2.5 text-sm text-textPrimary placeholder:text-textMuted transition-colors duration-200 focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20" />
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-4 pt-1">
            <label className="flex items-center gap-1.5 text-sm text-textPrimary">
              <input type="radio" name="floss" value="yes" className="h-4 w-4 border-border text-primary focus:ring-primary/30" />
              Yes
            </label>
            <label className="flex items-center gap-1.5 text-sm text-textPrimary">
              <input type="radio" name="floss" value="no" className="h-4 w-4 border-border text-primary focus:ring-primary/30" />
              No
            </label>
          </div>
        </div>
        ...
        <div>
          <FieldLabel htmlFor="brush_frequency">How often do you brush your teeth?</FieldLabel>
          <div className="flex flex-wrap items-center gap-4 pt-1">
            {["1x", "2x", "3-4x/day"].map((option) => (
              <label key={option} className="flex items-center gap-1.5 text-sm text-textPrimary">
                <input type="radio" name="brush_frequency" value={option} className="h-4 w-4 border-border text-primary focus:ring-primary/30" />
                {option}
              </label>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-1">
            {["Light", "Moderate", "Heavy"].map((option) => (
              <label key={option} className="flex items-center gap-1.5 text-sm text-textPrimary">
                <input type="radio" name="calculus" value={option} className="h-4 w-4 border-border text-primary focus:ring-primary/30" />
                {option}
              </label>
            ))}
          </div>
      ...
      <div className="mt-6">
        ...
        <textarea id="notes" name="notes" rows={3} placeholder="Additional observations" className="w-full resize-none rounded-xl border border-border bg-background px-3.5 py-2.5 text-sm text-textPrimary placeholder:text-textMuted transition-colors duration-200 focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20" />
      </div>
                  ...
                  <tr key={row} className={`border-b border-border last:border-b-0 ${index % 2 === 1 ? "bg-background/40" : ""}`}>
                    ...
                    {[1, 2, 3, 4].map((column) => (
                      <td key={column} className="p-3">
                        <input name={`${fieldName}_${column}`} type="text" className="w-full rounded-lg border border-textPrimary bg-surface px-2 py-1.5 text-sm text-textPrimary focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20" />
                      </td>
                    ))}
                  </tr>
                );
      ...
      {selectedTooth !== null && (
        <ToothNoteModal toothNumber={selectedTooth} initialRecord={toothRecords[selectedTooth]} onSave={saveTooth} onClear={clearTooth} onClose={closeModal} />
      )}
    </form>
  );
}
